$(function(){
   ...
   $('#outer').on('click','.outer-btn>ul>li',function(){
       ...
       //汽车图片切换路径
       $carImg.attr('src',`../img/index/co${$c+1}_020000.png`);
       //汽车过来时其他汽车隐藏并回去
       $carImg.animate({left:0,opacity:1}).siblings().css({left:500,opacity:0});
       //$('#outer>.outer-bg>img').attr('src',`../img/index/bg${$c}.png`);
   })
    /***************************彭拜动力******************************/
    ...
    $('#space').click(function(e){
        ...
        if($tar.parent().get(0).nodeName=="LI"){
            ...
            $('.show-img>img').attr('src',`../img/index/${imgSrc}.jpg`);
        }
    });
    //点击屏幕边缘区隐藏显示内容
    ...
    $('#peizhi .peizhiLi-bg').mouseenter(function(){
        $(this).css('backgroundImage','url(../img/index/peizhi_listbg2.png)');
    });
    $('#peizhi .peizhiLi-bg').mouseleave(function(){
        $(this).css('backgroundImage','url(../img/index/peizhi_listbg.png)');
    });
    //点击弹出详情框
